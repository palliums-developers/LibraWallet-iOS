//
//  DepositTokensAlert.swift
//  LibraWallet
//
//  Created by wangyingdong on 2020/8/27.
//  Copyright © 2020 palliums. All rights reserved.
//

import UIKit

class DepositTokensAlert: UIView {
    ...
    convenience init(data: [BankDepositMarketDataModel], successClosure: @escaping successClosure) {
    ...
    lazy var tableView: UITableView = {
        ...
        tableView.register(TokensCell.classForCoder(), forCellReuseIdentifier: "NormalCell")
        tableView.dataSource = self
        tableView.delegate = self
        return tableView
    }()
    typealias successClosure = (BankDepositMarketDataModel) -> Void
    ...
    var originModels: [BankDepositMarketDataModel]?
...
extension DepositTokensAlert: actionViewProtocol {
    
}
extension DepositTokensAlert: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let selectRow = pickerRow {
            ...
            if let oldCell = tableView.cellForRow(at: tempIndexPath) as? DepositTokensCell {
                oldCell.showSelectState = false
                indexPaths.append(tempIndexPath)
            }
        }
        let cell = tableView.cellForRow(at: indexPath) as! DepositTokensCell
...
extension DepositTokensAlert: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = "NormalCell"
        if let cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? DepositTokensCell {
            ...
            let cell = DepositTokensCell.init(style: UITableViewCell.CellStyle.default, reuseIdentifier: identifier)
            if let data = dataModels, data.isEmpty == false {
                cell.model = data[indexPath.row]
                cell.hideSpcaeLineState = (data.count - 1) == indexPath.row ? true:false
            }
...
extension DepositTokensAlert: actionViewAnimationProtocol {
    func showAnimation() {
