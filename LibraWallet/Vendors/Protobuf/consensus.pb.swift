...
struct Consensus_ConsensusMsg {
  ...
  var epochChange: Types_ValidatorChangeEventWithProof {
    get {
      if case .epochChange(let v)? = _storage._message {return v}
      return Types_ValidatorChangeEventWithProof()
    }
  ...
  enum OneOf_Message: Equatable {
    ...
    case epochChange(Types_ValidatorChangeEventWithProof)
  ...
  mutating func decodeMessage<D: SwiftProtobuf.Decoder>(decoder: inout D) throws {
    ...
    try withExtendedLifetime(_storage) { (_storage: _StorageClass) in
      while let fieldNumber = try decoder.nextFieldNumber() {
        ...
        case 6:
          var v: Types_ValidatorChangeEventWithProof?
          if let current = _storage._message {
            try decoder.handleConflictingOneOf()
