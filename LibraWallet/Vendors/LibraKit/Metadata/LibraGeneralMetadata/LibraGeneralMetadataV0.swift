...
struct LibraGeneralMetadataV0 {
    ...
    func serialize() -> Data {
        ...
        if referenced_event.isEmpty == false {
            result += Data.init(Array<UInt8>(hex: "01"))
            result += LibraUtils.getLengthData(length: NSDecimalNumber.init(string: self.referenced_event).uint64Value, appendBytesCount: 8)
        } else {
            result += Data.init(Array<UInt8>(hex: "00"))
        }
        return result
