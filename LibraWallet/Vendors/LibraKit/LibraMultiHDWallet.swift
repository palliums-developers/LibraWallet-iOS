...
struct LibraMultiHDWallet {
    ...
    ///   - privateKeys: 私钥数组
    ///   - threshold: 最少签名数
    init(privateKeys: [MultiPrivateKeyModel], threshold: Int, multiPublicKey: LibraMultiPublicKey? = nil) {
    ...
    init(models: [SeedAndDepth], threshold: Int, multiPublicKey: LibraMultiPublicKey? = nil) throws {
        ...
        for model in models {
            ...
            do {
                ...
                let privateModel = MultiPrivateKeyModel.init(raw: Data.init(bytes: privateKey, count: privateKey.count),
