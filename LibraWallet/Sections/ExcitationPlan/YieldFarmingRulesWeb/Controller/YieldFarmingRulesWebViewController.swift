//
//  YieldFarmingRulesWebViewController.swift
//  LibraWallet
//
...
class YieldFarmingRulesWebViewController: BaseViewController {
...
//        self.title = localLanguage(keyString: "wallet_service_navigation_title")
    ...
    private lazy var detailView : YieldFarmingRulesWebView = {
        let view = YieldFarmingRulesWebView.init()
        view.webView.navigationDelegate = self
        return view
    }()
    var needDismissViewController: Bool?
}
extension YieldFarmingRulesWebViewController :WKNavigationDelegate{
    // 页面开始加载时调用
