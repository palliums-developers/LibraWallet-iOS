//
//  ProfitInvitationViewController.swift
//  LibraWallet
//
...
class ProfitInvitationViewController: BaseViewController {
    ...
    deinit {
        print("ProfitInvitationViewController销毁了")
    }
    /// 网络请求、数据模型
    lazy var dataModel: ProfitInvitationModel = {
        let model = ProfitInvitationModel.init()
        return model
    }()
    /// tableView管理类
    lazy var tableViewManager: ProfitInvitationTableViewManager = {
        let manager = ProfitInvitationTableViewManager.init()
        //        manager.delegate = self
        return manager
    }()
    /// 子View
    lazy var detailView : ProfitInvitationView = {
        let view = ProfitInvitationView.init()
...
extension ProfitInvitationViewController {
    func initKVO() {
        self.observer = dataModel.observe(\.dataDic, options: [.new], changeHandler: { [weak self](model, change) in
            ...
            if type == "GetInviteProfitListOrigin" {
                guard let tempData = dataDic.value(forKey: "data") as? [InviteProfitDataModel] else {
                    return
                }
...
//                }
            ...
            } else if type == "GetInviteProfitListMore" {
                guard let tempData = dataDic.value(forKey: "data") as? [InviteProfitDataModel] else {
                    return
                }
...
extension ProfitInvitationViewController: JXSegmentedListContainerViewListDelegate {
