//
//  ProfitPoolViewController.swift
//  LibraWallet
//
...
class ProfitPoolViewController: BaseViewController {
    ...
    deinit {
        print("ProfitPoolViewController销毁了")
    }
    /// 网络请求、数据模型
    lazy var dataModel: ProfitPoolModel = {
        let model = ProfitPoolModel.init()
        return model
    }()
    /// tableView管理类
    lazy var tableViewManager: ProfitPoolTableViewManager = {
        let manager = ProfitPoolTableViewManager.init()
        //        manager.delegate = self
        return manager
    }()
    /// 子View
    lazy var detailView : ProfitPoolView = {
        let view = ProfitPoolView.init()
...
extension ProfitPoolViewController {
    func initKVO() {
        self.observer = dataModel.observe(\.dataDic, options: [.new], changeHandler: { [weak self](model, change) in
            ...
            if type == "GetPoolProfitListOrigin" {
                guard let tempData = dataDic.value(forKey: "data") as? [PoolProfitDataModel] else {
                    return
                }
...
//                }
            ...
            } else if type == "GetPoolProfitListMore" {
                guard let tempData = dataDic.value(forKey: "data") as? [PoolProfitDataModel] else {
                    return
                }
...
extension ProfitPoolViewController: JXSegmentedListContainerViewListDelegate {
