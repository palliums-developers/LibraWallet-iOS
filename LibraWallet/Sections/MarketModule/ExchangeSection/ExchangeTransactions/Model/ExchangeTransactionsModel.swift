...
struct ExchangeTransactionsDataModel: Codable {
    /// 输入数量
    var input_amount: UInt64?
    /// 兑换数量
    var output_amount: UInt64?
    /// 输入币名
    ...
    var gas_used: UInt64?
    ///
    var input_show_name: String?
    ///
    var output_show_name: String?
    /// 状态（同链上状态）
    var status: String?
    /// 交易唯一ID
    var version: Int?
...
//                        self?.setValue(data, forKey: "dataDic")
    ...
    deinit {
        ...
        requests.removeAll()
        print("ExchangeTransactionsModel销毁了")
    }
}
// MARK: 映射交易记录
extension ExchangeTransactionsModel {
