//
//  ScanViolasExcitationView.swift
//  LibraWallet
//
...
class ScanViolasExcitationView: UIView {
    ...
    deinit {
        print("ScanViolasExcitationView销毁了")
    }
    //MARK: - 布局
    ...
    lazy var scanLoginTitleLabel: UILabel = {
        ...
        label.text = localLanguage(keyString: "wallet_wallet_connect_bank_deposit_title")
    ...
    lazy var inputAmountTitleLabel: UILabel = {
        ...
        label.text = localLanguage(keyString: "wallet_profit_bank_market_withdraw_excitation_title")
        label.numberOfLines = 0
        return label
    }()
...
//        label.font = UIFont.systemFont(ofSize: 12, weight: .regular)
    ...
    lazy var confirmButton: UIButton = {
        ...
        button.setTitle(localLanguage(keyString: "wallet_wallet_connect_bank_market_withdraw_excitation_button_title"), for: UIControl.State.normal)
    ...
    var model: WCRawTransaction? {
        didSet {
            if model?.payload?.code == ViolasManager.getLocalMoveCode(bundle: "MarketContracts", contract: "withdraw_mine_reward") {
                scanLoginTitleLabel.text = localLanguage(keyString: "wallet_wallet_connect_market_excitation_title")
                inputAmountLabel.text = localLanguage(keyString: "wallet_profit_market_withdraw_excitation_content")
            } else if model?.payload?.code == ViolasManager.getLocalMoveCode(bundle: "BankContracts", contract: "claim_incentive") {
                scanLoginTitleLabel.text = localLanguage(keyString: "wallet_wallet_connect_bank_excitation_title")
                inputAmountLabel.text = localLanguage(keyString: "wallet_profit_bank_withdraw_excitation_content")
            }
