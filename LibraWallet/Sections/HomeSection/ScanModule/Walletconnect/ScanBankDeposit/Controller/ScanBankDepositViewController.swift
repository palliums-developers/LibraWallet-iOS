//
//  ScanBankDepositViewController.swift
//  LibraWallet
//
...
class ScanBankDepositViewController: UIViewController {
    ...
    deinit {
        print("ScanBankDepositViewController销毁了")
    }
    override func viewWillAppear(_ animated: Bool) {
        super.viewWillAppear(animated)
        self.navigationController?.navigationBar.barStyle = .default
    }
    private lazy var detailView : ScanBankDepositView = {
        let view = ScanBankDepositView.init()
        view.delegate = self
        return view
    }()
    lazy var dataModel: ScanBankDepositModel = {
        let model = ScanBankDepositModel.init()
        return model
    }()
    /// 数据监听KVO
...
extension ScanBankDepositViewController {
    func initKVO() {
        self.observer = dataModel.observe(\.dataDic, options: [.new], changeHandler: { [weak self](model, change) in
            ...
            if type == "SendViolasBankDepositTransaction" {
                self?.detailView.toastView.hide(tag: 99)
...
extension ScanBankDepositViewController: ScanSendTransactionViewDelegate {
    func cancelLogin() {
        ...
        if let raw = self.model {
            WalletManager.unlockWallet(controller: self) { [weak self] (result) in
                ...
                case let .success(mnemonic):
                    ...
                    self?.dataModel.sendDepositTransaction(model: raw,
                                                           mnemonic: mnemonic,
