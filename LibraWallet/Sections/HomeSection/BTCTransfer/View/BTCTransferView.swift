...
import UIKit
protocol BTCTransferViewDelegate: NSObjectProtocol {
    func scanAddressQRcode()
    func chooseAddress()
    func confirmTransfer(amount: Double, address: String, fee: Double)
}
class BTCTransferView: UIView {
    weak var delegate: BTCTransferViewDelegate?
    ...
    var wallet: LibraWalletManager? {
        didSet {
            ...
            let balance = String.init(format: "%.8f", (Double(model.walletBalance ?? 0) / 100000000.0))
